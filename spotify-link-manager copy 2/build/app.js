document.addEventListener('DOMContentLoaded', () => {
    ...
    let coverArtCache = {}; // cache fetched thumbnails by playlist link
    ...
    const changelog = [
        ...
        { filename: 'v1.5-cover-art.md', desc: 'Cover art preview inside pills — oEmbed fetch, hover reveal, luminosity blend' }
    ];
    ...
    function createPlaylistRow(playlist, index, curatorContent) {
        ...
        // Cover art preview (fetched async, fades in on hover via CSS)
        const coverImg = document.createElement('img');
        coverImg.className = 'pill-cover-art';
        coverImg.alt = '';
        pillLink.appendChild(coverImg);
        fetchCoverArt(playlist.link, coverImg);
    ...
    // ========================================
    // COVER ART FETCH (oEmbed)
    // ========================================
    async function fetchCoverArt(link, imgEl) {
        if (coverArtCache[link]) {
            imgEl.src = coverArtCache[link];
            return;
        }
        try {
            ...
            if (res.ok) {
                ...
                if (data.thumbnail_url) {
                    coverArtCache[link] = data.thumbnail_url;
                    imgEl.src = data.thumbnail_url;
                }
            }
        } catch (e) {
            // silently fail — pill works fine without art
        }
    }
