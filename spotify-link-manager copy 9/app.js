document.addEventListener('DOMContentLoaded', () => {
    ...
    if (actionSortName) actionSortName.addEventListener('click', () => {
        ...
        items.sort((a, b) => {
            const na = a.querySelector('.playlist-text')?.textContent || '';
            const nb = b.querySelector('.playlist-text')?.textContent || '';
            return nameAZ ? na.localeCompare(nb) : nb.localeCompare(na);
        });
        items.forEach(item => dropZone.appendChild(item));
        nameAZ = !nameAZ;
        actionSortName.title = nameAZ ? 'Sort by Name A-Z' : 'Sort by Name Z-A';
    });
    ...
    if (actionSortCurator) actionSortCurator.addEventListener('click', () => {
        ...
        if (items.length === 0) return;
        items.sort((a, b) => {
            const ca = a.querySelector('.curator-value')?.textContent || '';
            const cb = b.querySelector('.curator-value')?.textContent || '';
            return curatorAZ ? ca.localeCompare(cb) : cb.localeCompare(ca);
        });
        items.forEach(item => dropZone.appendChild(item));
        curatorAZ = !curatorAZ;
        actionSortCurator.querySelector('.dot-icon').textContent = curatorAZ ? 'C' : 'C';
        actionSortCurator.title = curatorAZ ? 'Sort by Curator A-Z' : 'Sort by Curator Z-A';
    });
    ...
    if (actionSortTag) actionSortTag.addEventListener('click', () => {
        ...
        if (items.length === 0) return;
        items.sort((a, b) => {
            const ta = a.querySelector('.subtle-tag')?.textContent || '';
            const tb = b.querySelector('.subtle-tag')?.textContent || '';
            return tagAZ ? ta.localeCompare(tb) : tb.localeCompare(ta);
        });
        items.forEach(item => dropZone.appendChild(item));
        tagAZ = !tagAZ;
        actionSortTag.title = tagAZ ? 'Sort by Tag A-Z' : 'Sort by Tag Z-A';
    });
    ...
    // =====================================================
    //  PRESET PLAYLIST SETS (ghost dot shortcuts)
    // =====================================================
    const presetSets = {
        1: ['Deep House Relax', 'Lo-Fi Beats', 'Coffee Table Jazz'],
        2: { tags: ['House', 'Tech House'] },
        3: { curators: ['brandon', 'peter'], tags: ['House', 'Tech House'] },
        4: [],
        5: [],
        6: [],
        7: [],
    ...
    document.querySelectorAll('.ghost-dot').forEach(dot => {
        ...
        dot.addEventListener('click', () => {
            ...
            // Filter-based matching (tags and/or curators)
            if (preset.tags || preset.curators) {
                playlistData["Top 20 Overall"].forEach(p => {
                    const tagMatch = preset.tags && p.tag && preset.tags.some(t => t.toLowerCase() === p.tag.toLowerCase());
                    const curatorMatch = preset.curators && p.curator && preset.curators.some(c => c.toLowerCase() === p.curator.toLowerCase());
                    if (tagMatch || curatorMatch) addSelectedPlaylist(p);
                });
                return;
            }

            // Name-based matching (array of names)
            if (Array.isArray(preset)) {
                preset.forEach(name => {
                    const playlist = playlistData["Top 20 Overall"].find(p => p.name === name);
                    if (playlist) addSelectedPlaylist(playlist);
                });
            }
        });
    });
