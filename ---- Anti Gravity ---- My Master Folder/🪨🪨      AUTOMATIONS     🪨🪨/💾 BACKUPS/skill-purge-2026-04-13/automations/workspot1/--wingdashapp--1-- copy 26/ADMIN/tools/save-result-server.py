...
# ── Unified client folder mapping ──────────────────────────────────────
UNIFIED_SUBFOLDER_MAP = {
    'homebases':     'web-app',
    'presentations': 'presentations',
    'payment-cards': 'payments',
}

def resolve_folder(rtype, client_slug, unified=False, subfolder=''):
    """Return the correct folder path under ADMIN_ROOT.
    If unified=True, routes to ADMIN/clients/{slug}/{mapped-subfolder}/
    Otherwise keeps legacy ADMIN/{type}/{slug}/ path.
    """
    if unified and rtype in UNIFIED_SUBFOLDER_MAP:
        mapped = UNIFIED_SUBFOLDER_MAP[rtype]
        base = os.path.join(ADMIN_ROOT, 'clients', client_slug, mapped)
    else:
        base = os.path.join(ADMIN_ROOT, rtype, client_slug)
...
class AdminHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == '/check':
            ...
            self._handle_list()
        elif self.path == '/ensure-client-folder':
            self._handle_ensure_client_folder()
        else:
            self._json_response(404, {'error': 'Not found'})
    ...
    def _handle_check(self):
        ...
        try:
            ...
            folder_path = resolve_folder(rtype, client_slug, unified=unified, subfolder=subfolder)
    ...
    def _handle_list(self):
        ...
        try:
            ...
            client_slug = sanitize_slug(client)
            unified = data.get('unified', False)
            folder_path = resolve_folder(rtype, client_slug, unified=unified)
    ...
    def _handle_ensure_client_folder(self):
        """Create unified client folder structure under ADMIN/clients/{slug}/.
        Body: { "client": "Client Name", "id": "CL-xxx" (optional) }
        Returns: { "success": true, "path": "...", "created": bool }
        ...
        try:
            ...
            if not client:
                self._json_response(400, {'error': 'Missing client name'})
                return

            client_slug = sanitize_slug(client)
            client_root = os.path.join(ADMIN_ROOT, 'clients', client_slug)
            already_existed = os.path.isdir(client_root)

            # Create all subfolders
            for sub in ['web-app', 'presentations', 'payments', 'proposals']:
                os.makedirs(os.path.join(client_root, sub), exist_ok=True)

            # Write/update client.json metadata
            meta_path = os.path.join(client_root, 'client.json')
            if os.path.isfile(meta_path):
                with open(meta_path, 'r', encoding='utf-8') as f:
                    meta = json.loads(f.read())
                # Update name/id if provided but don't overwrite created
                meta['name'] = client
                if client_id:
                    meta['id'] = client_id
            ...
            if not already_existed:
                print(f'  [CLIENT] Created folder: clients/{client_slug}/')
            else:
                print(f'  [CLIENT] Verified folder: clients/{client_slug}/')

            self._json_response(200, {
                'success': True,
                'path': client_root.replace('\\', '/'),
                'slug': client_slug,
                'created': not already_existed
            })

        except Exception as e:
            print(f'  [ERROR] ensure-client-folder: {e}')
            self._json_response(500, {'error': str(e)})
...
if __name__ == '__main__':
    ...
    for folder in ['homebases', 'presentations', 'payment-cards', 'templates', 'clients']:
    ...
    print('    POST /ensure-client-folder — create unified client folder')
