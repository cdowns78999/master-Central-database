...
│   ├── build-digital-coms.html       ← Build Guide (source of truth, step definitions)
...
5. **Create job green-sheet** — for the selected job (or all jobs in scope), if one doesn't already exist:
    - Copy `green-spreadsheets-TEMPLATE.html` from `⚙️ ADMIN/green spread folder base/`
...
6. **Report status** in 2-3 lines: steps completed so far, current position, backup created, green-sheet status. Thread to STEP 1.
...
**Registry feature:** Per-job output in two modes:
...
**Force-chain:** After Plan 1 changes are complete and verified, immediately proceed to Plan 2 execution unless Claude needs Chad's input on a specific decision. Signal: "Plan 1 complete → rolling into Plan 2"
