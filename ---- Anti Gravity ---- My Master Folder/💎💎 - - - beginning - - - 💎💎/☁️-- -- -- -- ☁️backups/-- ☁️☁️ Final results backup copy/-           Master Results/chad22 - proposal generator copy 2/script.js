...
function exportProposalJSON() {
    const payload = {
        ...
        commercials: {
            orders: state.orders,
            discount: dom.discountInput.value,
            bonus: [dom.bonus1.value, dom.bonus2.value],
            deadline: dom.deadlineDate.value,
    ...
    downloadAnchorNode.setAttribute("download", `proposal_${state.artist.name.replace(/\s+/g, '_') || 'export'}.json`);
    document.body.appendChild(downloadAnchorNode);
    downloadAnchorNode.click();
    downloadAnchorNode.remove();
}

function importProposalJSON() {
...
function restoreFromJSON(data) {
    if (data.metadata) state.artist = data.metadata;
    if (data.vision) {
        dom.field1.value = data.vision.strategy || '';
        dom.field2.value = data.vision.timeline || '';
    }
    if (data.commercials) {
        state.orders = data.commercials.orders || [];
        dom.discountInput.value = data.commercials.discount || 0;
        if (data.commercials.bonus) {
            dom.bonus1.value = data.commercials.bonus[0] || '';
            dom.bonus2.value = data.commercials.bonus[1] || '';
        }
        dom.deadlineDate.value = data.commercials.deadline || '';
    }

    updateArtistUI();
    updateLedgerUI();
    masterGenerate(); // Auto-visualize on import
    alert("Proposal Imported Successfully!");
}

function applyShortcut(type, event) {
    const shortcuts = {
        starter: [
            { name: '10K Spotify', amount: '$325', type: 'spotify' },
            { name: '3 Articles (In-Network)', amount: '$200', type: 'press' }
        ...
        elite: [
            { name: '250K Spotify', amount: '$3,550', type: 'spotify' },
            { name: '6 Articles (Premium)', amount: '$750', type: 'press' },
            { name: '500K SoundCloud', amount: '$6,500', type: 'soundcloud' } // Note: SoundCloud DB max is 16K but elite is custom
        ]
    };

    if (shortcuts[type]) {
        state.orders = [...shortcuts[type]];
        updateLedgerUI();
