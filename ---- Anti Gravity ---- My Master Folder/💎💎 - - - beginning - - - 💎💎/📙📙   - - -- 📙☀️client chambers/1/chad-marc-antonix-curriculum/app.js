...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderStar4(starKey) {
        ...
        // ── Phase Cards (the playbook — chad31 design) ──
        star.phases.forEach(phase => {
            html += `
                <div class="section-header">
                    <div class="section-dot" style="background:${accent};"></div>
                    <span class="section-title">${phase.label}: ${phase.title}</span>
                </div>
            `;

            phase.steps.forEach(step => {
                // Format body text: \n → <br>, style bullets and numbered lists
                ...
                // Source link bar (real links, not placeholders)
