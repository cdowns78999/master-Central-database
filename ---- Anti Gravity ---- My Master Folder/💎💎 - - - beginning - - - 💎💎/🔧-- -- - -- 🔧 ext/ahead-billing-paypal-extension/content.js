// ── Pass Off Claude Max Extension 1 ──
// Self-contained billing field builder for PayPal pages
...
(function () {
  ...
  function showInput(mode) {
    ...
    const placeholder = isInv
      ? 'Artist / Item / Price'
      : 'Artist / Plan / Price / Cycle';
    const example = isInv
      ? 'DJ Nova / Beat License / $200'
  ...
  function parseAndConfirm() {
    ...
    if (STATE.mode === 'invoice') {
      ...
      const price = parts[2] || '';
      STATE.fields = [
        { label: 'CUSTOMER NAME', value: artist },
        { label: 'ITEM NAME', value: item },
        { label: 'PRICE PER UNIT', value: price },
        { label: 'DESCRIPTION', value: `${item} for ${artist} — Ahead Artist Solutions` }
      ];
    } else {
      const artist = parts[0] || '';
      const plan = parts[1] || '';
      ...
      STATE.fields = [
        { label: 'PLAN NAME', value: `${plan} — ${artist}` },
        { label: 'DESCRIPTION', value: `${plan} services for ${artist} provided by Ahead Artist Solutions` },
  ...
  function showReadyScreen() {
    ...
    const alreadyThere = window.location.href.includes(
      isInv ? '/invoice/create' : '/billing/plans'
    );
    ...
    panel.innerHTML = `
      ...
      <div id="passoff-status">${alreadyThere ? 'click to see form guide' : 'takes you to paypal + loads guide'}</div>
    `;
    bindClose();
    panel.querySelector('#passoff-fire').addEventListener('click', () => {
      ...
      if (alreadyThere) {
        ...
        // Navigate to the correct PayPal page — data will reload from localStorage
  ...
  // ── Render: Form Guide (mini PayPal mockup) ──
  function showFormGuide() {
    ...
    if (isInv) {
      ...
      const cycleRaw = f[2].value.toLowerCase();
      ...
      const priceClean = f[3].value.replace(' USD','');
      ...
      STATE.subPages = [
        `<div class="pp-mock">
          ...
          <div class="pp-mock-step">
            ...
            <div class="pp-step-label">Choose product</div>
          </div>
          <div class="pp-mock-dimmed">Select your existing product</div>
          <div class="pp-mock-dimmed">or "Create a subscription product"</div>
          <div class="pp-mock-step-spacer"></div>
          <div class="pp-mock-dimmed">Then pick <b>Fixed pricing</b></div>
        ...
        `<div class="pp-mock">
          ...
          <div class="pp-mock-step">
            ...
            <div class="pp-step-label">Create plan</div>
          </div>
          <div class="pp-mock-field pp-hot" data-value="${escapeAttr(f[0].value)}">
            <div class="pp-mock-label">Plan name</div>
            <div class="pp-mock-value">${escapeHtml(f[0].value)}</div>
          </div>
          <div class="pp-mock-field pp-hot" data-value="${escapeAttr(f[1].value)}">
        ...
        `<div class="pp-mock">
          ...
          <div class="pp-mock-step">
            ...
            <div class="pp-step-label">Define pricing</div>
          </div>
          ...
          <div class="pp-mock-dimmed">Unlimited billing cycles</div>
          <div class="pp-mock-btn">Next →</div>
        </div>`,
        ...
        `<div class="pp-mock">
          ...
          <div class="pp-mock-step">
            ...
            <div class="pp-step-label">Review and save</div>
          </div>
          <div class="pp-mock-dimmed">Plan: ${escapeHtml(f[0].value)}</div>
          <div class="pp-mock-dimmed">Price: ${escapeHtml(priceClean)} USD / ${cyclePeriod.toLowerCase()}</div>
          <div class="pp-mock-dimmed">Cycles: Unlimited</div>
          <div class="pp-mock-step-spacer"></div>
          <div class="pp-mock-dimmed">Verify → Turn plan on</div>
          <div class="pp-mock-btn">Save & Turn On →</div>
        </div>`
      ];
      STATE.subPage = 0;
      showSubPage();
  ...
  function sendCompletionSignal() {
    ...
    if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.sendMessage) {
      chrome.runtime.sendMessage({ type: 'passoff_complete', data: resultData }, function() {
        console.log('[PayPal] Completion signal sent');
      });
    }
