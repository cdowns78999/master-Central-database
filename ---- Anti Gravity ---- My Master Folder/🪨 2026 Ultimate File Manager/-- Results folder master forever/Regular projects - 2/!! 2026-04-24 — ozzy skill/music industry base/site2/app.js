...
function srcBanner(catId, tileNum) {
  if (typeof TILE_SOURCES === 'undefined') return '';
  const srcs = TILE_SOURCES[`${catId}-${tileNum}`];
  if (!srcs || !srcs.length) return '';
  return `<div class="tile-source-banner">${srcs.map(s =>
    `<a href="${s.u}" target="_blank" rel="noopener" title="${s.t}">${s.t}</a>`
  ).join('')}</div>`;
}
