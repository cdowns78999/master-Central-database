const app = {
    ...
    init() {
        ...
        // Handle global long press cleanup
    ...
    startLongPress(el, callback) {
        this.cancelLongPress();
        el.classList.add('long-pressing'); // Visual feedback if we had a class
        this.longPressTimer = setTimeout(() => {
            el.classList.remove('long-pressing');
            el.classList.add('long-press-feedback');
            setTimeout(() => el.classList.remove('long-press-feedback'), 500);
            callback();
        }, 800);
    },
    ...
    load(screenName) {
        ...
        setTimeout(() => {
            if (SCREENS[screenName]) {
                ...
                const navMap = { 'home': 0, 'ticket': 3, 'profile': 5 };
        ...
        const weekCards = this.weeklyData.map((w, idx) => `
            <div class="glass-medium week-card active-glow" data-week="${idx}" style="min-height: 140px; border-radius: 20px; padding: 16px; position: relative; overflow: hidden; cursor: pointer;" onclick="app.toggleWeekSheet(${idx})">
                ...
                <div style="position: relative; display: flex; justify-content: space-between; align-items: flex-start;">
                    ...
                    <div class="weekly-date-carousel" onclick="event.stopPropagation()">
                        <div class="weekly-cycle-dot" onclick="event.stopPropagation(); app.cycleWeeklyDate(${idx})"></div>
                        <div class="weekly-date-single" 
                             onmousedown="app.startLongPress(this, () => app.enableEdit(this))"
                             onmouseup="app.cancelLongPress()"
                             onmouseleave="app.cancelLongPress()"
                             ontouchstart="event.stopPropagation(); app.startLongPress(this, () => app.enableEdit(this))"
                             ontouchend="app.cancelLongPress()"
                             spellcheck="false"
                             onblur="app.updateWeekDate(${idx}, this.innerText); this.contentEditable='false'">${w.date}</div>
                    </div>
                </div>
        ...
        this.weeklyData.forEach((w, idx) => {
            ...
            mount.innerHTML = tasks.map((task, i) => `
                <div class="recap-task-row" onclick="event.stopPropagation(); app.enterFocusMode('week', ${idx * 10 + i}, '${task.replace(/'/g, "\\'")}')">
                    <div class="task-dot"></div>
                    <div class="task-input" style="cursor: pointer;">${task || '<span class="task-placeholder">Task #' + (i + 1) + '</span>'}</div>
                </div>
            `).join('');
        });
    },
    ...
    renderWhisperSlots() {
        ...
        mount.innerHTML = this.whisperData.map((d, i) => {
            ...
            return `<div class="${className}" 
                         onmousedown="event.stopPropagation(); app.startLongPress(this, () => app.enableEdit(this))"
                         onmouseup="app.cancelLongPress()"
                         onmouseleave="app.cancelLongPress()"
                         ontouchstart="event.stopPropagation(); app.startLongPress(this, () => app.enableEdit(this))"
                         ontouchend="app.cancelLongPress()"
                         onblur="app.updateWhisperDate(${i}, this.innerText); this.contentEditable='false'">${d.date}</div>`;
        }).join('');
    },
    ...
    updateWhisperDate(idx, newDate) {
        if (this.whisperData[idx]) {
            this.whisperData[idx].date = newDate;
            this.showFeedback('Recap Date Updated');
            this.renderWhisperSlots();
        }
    },
    ...
    renderWhisperTasks() {
        ...
        mount.innerHTML = currentTasks.map((task, i) => `
            <div class="recap-task-row" onclick="event.stopPropagation(); app.enterFocusMode('whisper', ${i}, '${task}')">
                <div class="task-dot"></div>
                <div class="task-input" style="cursor: pointer;">${task || '<span class="task-placeholder">Vision #' + (i + 1) + '</span>'}</div>
            </div>
        `).join('');
    },
    ...
    exportData() {
        ...
        const fileName = `on-the-move-recap-${new Date().toISOString().slice(0, 10)}.json`;
        ...
        this.showFeedback('Recap Exported');
    },
    ...
    importData(input) {
        ...
        reader.onload = (e) => {
            try {
                ...
                if (!imported.weeklyData || !imported.whisperData) {
                    throw new Error("Invalid Recap data format");
                }

                this.weeklyData = imported.weeklyData;
                this.whisperData = imported.whisperData;
                ...
                this.showFeedback('Recap Imported');
                console.log("On-the-Move Data imported successfully");
            } catch (err) {
                console.error("Import failed:", err);
                this.showFeedback('Import Failed');
    ...
    toggleFontScaler() {
        const tray = document.getElementById('font-scaler-tray');
        const btn = document.getElementById('font-scaler-btn');
        if (tray) {
    ...
    toggleTextSizer() {
        const popup = document.getElementById('text-sizer-popup');
        popup.classList.toggle('active');

        // Auto-close after 3 seconds of inactivity
        clearTimeout(this.textSizerTimeout);
        this.textSizerTimeout = setTimeout(() => {
    ...
    updateTextSize(val) {
        const base = 0.76;
        const step = 0.1;
        const newSize = base + (val * step);

        // Update CSS variables
        document.documentElement.style.setProperty('--task-font-size', `${newSize}rem`);
        document.documentElement.style.setProperty('--task-text-color', '#ffffff');
        const dotSize = Math.max(6, newSize * 15.8);
        document.documentElement.style.setProperty('--task-dot-size', `${dotSize}px`);

        // Keep text sizer open while adjusting
        clearTimeout(this.textSizerTimeout);
        this.textSizerTimeout = setTimeout(() => {
            document.getElementById('text-sizer-popup').classList.remove('active');
        }, 3000);
    }
};
