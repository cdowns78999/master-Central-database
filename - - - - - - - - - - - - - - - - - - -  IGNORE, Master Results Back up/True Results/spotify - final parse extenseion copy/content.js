    ...
    tile.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <div style="display: flex; align-items: center; gap: 10px;">
                ...
                <h2 style="margin: 0; font-size: 16px; font-weight: 800; letter-spacing: -0.5px;">On Mode</h2>
            </div>
        ...
        <p style="margin: 0; font-size: 13px; color: #3a3a3c; line-height: 1.5; font-weight: 450;">
            ${statusText}
        </p>
        <div style="background: rgba(0,122,255,0.05); padding: 15px; border-radius: 20px; border: 1px solid rgba(0,122,255,0.1); margin: 5px 0;">
            ...
            <input type="text" id="global-artist-id-input" placeholder="Optional: Enter Artist ID..." 
                style="width: 100%; background: transparent; border: none; font-size: 12px; outline: none; color: #1c1c1e;"
                value="${globalArtistId}">
        </div>
        <button id="scan-battle-btn" class="ios-btn" style="height: 52px; font-size: 14px;">
            ${buttonText}
        </button>
    `;
...
function showBattleCommand(tile) {
    ...
    let rowsHtml = clusters.map(c => `
        <tr class="cluster-row" data-index="${c.index}" style="border-bottom: 1px solid rgba(0,0,0,0.03);">
            ...
            <td style="padding: 10px 10px; font-weight: 700; font-size: 12px; color: #1c1c1e;">${c.artist}</td>
            <td style="padding: 10px 10px; font-size: 12px; color: #3a3a3c;">${c.song}</td>
            <td style="padding: 10px 10px; text-align: center;">
                <button class="ios-btn go-btn" style="height: 32px; width: 60px; font-size: 11px;">GO</button>
            </td>
        </tr>
    `).join('');
    ...
    tile.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <div style="display: flex; align-items: center; gap: 10px;">
                ...
                <h2 style="margin: 0; font-size: 16px; font-weight: 800; letter-spacing: -0.5px;">Battle Command</h2>
            </div>
            <div style="display: flex; align-items: center; gap: 10px;">
                <button id="battle-all-btn" class="ios-btn" style="height: 32px; padding: 0 12px; font-size: 10px; background: #007aff;">⚔️ BATTLE ALL</button>
                <a href="https://drive.google.com/drive/folders/1qyzU7k_FuGsptOi1NwchJ7Li1dGbAO6c" target="_blank" 
                   style="font-size: 10px; font-weight: 700; color: #007aff; text-decoration: none; background: rgba(0,122,255,0.1); padding: 6px 10px; border-radius: 12px;">📂 DRIVE</a>
                <button id="rescan-battle-btn" style="background: rgba(0,0,0,0.05); border: none; border-radius: 12px; padding: 6px 10px; font-size: 10px; font-weight: 700; color: #8e8e93; cursor: pointer;">RESCAN</button>
                <button id="close-battle-btn" style="background: transparent; border: none; font-size: 26px; color: #8e8e93; cursor: pointer;">&times;</button>
            </div>
        </div>
        ...
        <div id="cluster-scroll-container" style="max-height: 480px; overflow-y: auto; background: rgba(0,0,0,0.02); border-radius: 24px;">
            <table style="width: 100%; border-collapse: collapse; text-align: left;">
                <thead style="position: sticky; top: 0; background: rgba(255,255,255,0.9); backdrop-filter: blur(10px); z-index: 2;">
                    <tr>
                        <th style="padding: 10px 10px; font-size: 9px; color: #8e8e93; text-transform: uppercase; letter-spacing: 0.8px; text-align: center; width: 50px;">Status</th>
                        <th style="padding: 10px 10px; font-size: 9px; color: #8e8e93; text-transform: uppercase; letter-spacing: 0.8px;">Artist</th>
                        <th style="padding: 10px 10px; font-size: 9px; color: #8e8e93; text-transform: uppercase; letter-spacing: 0.8px;">Song</th>
                        <th style="padding: 10px 10px; font-size: 9px; color: #8e8e93; text-transform: uppercase; letter-spacing: 0.8px; text-align: center; width: 80px;">Action</th>
                    </tr>
                </thead>
                <tbody>
                    ${rowsHtml}
...
async function processCluster(cluster) {
    ...
    try {
        ...
        if (!folderResponse || !folderResponse.success) {
            ...
            if (folderResponse?.error?.includes('bad client id')) {
                alert("🚨 GOOGLE AUTH ERROR: The 'Client ID' in your manifest.json is invalid or has expired.\n\nTo fix this:\n1. Go to Google Cloud Console\n2. Update your OAuth Client ID with your current Extension ID\n3. Copy the new Client ID into manifest.json\n4. Reload the extension.");
            }

            return { success: false, error: folderResponse?.error };
        }
