...
const DATA_READ_RANGE = "B136:H236";
...
async function handleScrapeAndSync(request, sendResponse) {
    ...
    try {
        ...
        const resultRecord = {
            ...
            algoRatio: `${algoRatio}% ⭐`,
            streamsLeft,
            playlistCount: `${data1yr.playlistCount} Playlists`,
            daysLeft
        };
        ...
        const resultRow = [
            "X", // Col I (Marker)
            resultRecord.daily,
            resultRecord.playlistTotal,
            resultRecord.algoStreams,
            resultRecord.algoRatio,
            resultRecord.streamsLeft,
            resultRecord.playlistCount,
            resultRecord.daysLeft,
            "" // Col Q (Buffer)
        ];
...
chrome.action.onClicked.addListener(async () => {
    ...
    try {
        const token = await getAuthToken();
        const response = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/${DATA_READ_RANGE}`, {
