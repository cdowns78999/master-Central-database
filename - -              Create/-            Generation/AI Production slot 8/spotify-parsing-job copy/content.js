...
function showBattleCommand(tile) {
    ...
    tile.style.width = "620px";
    const clusters = Array.from(cards).map((card, index) => ({
        index,
...
async function processCluster(cluster) {
    ...
    // Use global override if available, otherwise card dataset (strictly trimmed)
    const artistId = (globalArtistId || card.dataset.artistId || '').trim();
    const songId = (card.dataset.songId || '').trim();

    console.log(`Processing: ${folderName}`);
    console.log(`Artist ID: |${artistId}| ${globalArtistId ? '(OVERRIDDEN)' : ''}, Song ID: |${songId}|`);

    if (!artistId || !songId) {
        console.error("Missing artistId or songId in card dataset");
        return false;
    }
