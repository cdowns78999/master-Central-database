console.log("Spotify Parsing Job: Master Edition Content Script Loaded");
let globalArtistId = '';
...
function parseSpreadsheetRows(rows) {
    return rows.map(row => {
        // Range A136:J236 (Capture exact first 10 columns for the 10-column Master String)
        const nameData = row[0] || "";
        const amount = row[1] || "";
        const colC = row[2] || "";
        const dailyStreams = row[3] || "";
        const totalIndie = row[4] || "";
        const algStreams = row[5] || "";
        const algRatio = row[6] || "";
        const streamsLeft = row[7] || "";
        const playlistCount = row[8] || "";
        const daysLeft = row[9] || "";

        // The 'Master String' is built from these exactly
        const masterString = [
            nameData, amount, colC, dailyStreams, totalIndie,
            algStreams, algRatio, streamsLeft, playlistCount, daysLeft
        ].join('\t');
...
function showBattleCommand(tile) {
            ...
            <table style="width: 100%; border-collapse: collapse; text-align: left;">
                <thead style="position: sticky; top: 0; background: rgba(255,255,255,0.9); backdrop-filter: blur(10px); z-index: 2;">
                    <tr>
                        ...
                        <th style="padding: 14px 10px; font-size: 10px; color: #8e8e93; text-transform: uppercase; letter-spacing: 0.8px;">Artist / Song</th>
                        <th style="padding: 14px 10px; font-size: 10px; color: #8e8e93; text-transform: uppercase; letter-spacing: 0.8px; text-align: center; width: 220px;">Action</th>
                    </tr>
                </thead>
                <tbody>
                    ${clusters.map(c => `
                        <tr class="cluster-row" data-index="${c.index}" style="border-bottom: 1px solid rgba(0,0,0,0.03);">
                            ...
                            <td style="padding: 16px 10px;">
                                <div style="font-weight: 700; font-size: 14px; color: #1c1c1e;">${c.artist}</div>
                                <div style="font-size: 12px; color: #8e8e93;">${c.song}</div>
                            </td>
                            <td style="padding: 16px 10px; text-align: center;">
                                <div style="display: flex; gap: 8px; justify-content: center;">
                                    <button class="ios-btn go-btn" style="height: 32px; width: 60px; font-size: 11px;">GO</button>
                                    <button class="ios-btn master-copy-btn" style="height: 32px; width: 100px; font-size: 10px; background: rgba(0,196,255,0.1); color: #007aff; border: 1px solid rgba(0,122,255,0.2);">COPY MASTER</button>
                                </div>
                            </td>
                        </tr>
                    `).join('')}
    ...
    tile.querySelectorAll('.master-copy-btn').forEach(btn => {
        btn.onclick = (e) => {
            const row = e.target.closest('.cluster-row');
            const c = clusters[row.dataset.index];
            if (!c.isSpreadsheet) {
                alert("Master Copy only works for Spreadsheet data.");
                return;
            }
            ...
            navigator.clipboard.writeText(masterString);
    ...
    document.getElementById('battle-all-btn').onclick = async () => {
        ...
        for (const row of rows) {
            ...
            if (!battleAbort && completed < rows.length) await new Promise(r => setTimeout(r, 1500));
        }
...
async function processCluster(cluster) {
    ...
    let artistId = globalArtistId || cluster.artistId || "";
    let songId = cluster.songId || "";

    if (!cluster.isSpreadsheet) {
        const card = cluster.originalCard;
        artistId = globalArtistId || card.dataset.artistId || "";
    ...
    try {
        ...
        if (!folderResponse || !folderResponse.success) return { success: false, error: folderResponse?.error };
        ...
        return batchResponse && batchResponse.success ? { success: true, folderId } : { success: false, error: batchResponse?.error };
    } catch (err) {
