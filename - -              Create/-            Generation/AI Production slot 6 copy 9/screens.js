home: `<!-- REFINED DASHBOARD (SLOT 6) -->
<div class="screen-animation-fade" style="padding-top: 35px;">
    
    <!-- Dashboard Status Bar -->
    <div style="display: flex; gap: 10px; margin-bottom: 25px; overflow-x: auto; padding-bottom: 5px;" class="scroll-hide">
        <div class="dashboard-pill"><span></span> ACTIVE GOALS</div>
        <div class="dashboard-pill" style="opacity: 0.6;"><span style="background: var(--accent-pink); box-shadow: 0 0 8px var(--accent-pink);"></span> NEXT PHASE</div>
        <div class="dashboard-pill" style="opacity: 0.6;"><span style="background: #32cd32; box-shadow: 0 0 8px #32cd32;"></span> SYNCED</div>
    </div>

    <!-- Featured Road Map Title (1 Row) -->
    <h1 style="font-size: 1.9rem; font-family: 'Outfit'; margin-bottom: 8px; line-height: 1.1; letter-spacing: -0.8px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
        'Live Your Dreams' Road Map <span style="display: inline-block; width: 10px; height: 10px; background: var(--accent-pink); border-radius: 50%; box-shadow: 0 0 15px var(--accent-pink); margin-left: 4px; animation: glowPulse 2s infinite;"></span>
    </h1>
    <p style="color: var(--text-ghost); font-size: 0.8rem; font-weight: 600; margin-bottom: 30px; opacity: 0.6; letter-spacing: 1px; text-transform: uppercase;">System Terminal | Node 06</p>

    <!-- Featured Module (Hero) -->
    <div class="glass-medium active-glow" style="position: relative; border-radius: var(--radius-lg); overflow: hidden; height: 160px; cursor: pointer; border-color: rgba(255,255,255,0.1);" onclick="app.load('detail')">
        <img src="https://images.unsplash.com/photo-1598488035139-bdbb2231ce04?q=80&w=1000" style="width: 100%; height: 100%; object-fit: cover; position: absolute; inset: 0; filter: contrast(1.1) brightness(0.8);">
        <div style="position: absolute; inset: 0; background: linear-gradient(135deg, rgba(26, 11, 46, 0.9) 0%, rgba(75, 76, 237, 0.4) 100%);"></div>
        <div style="position: absolute; inset: 0; padding: 25px; display: flex; flex-direction: column; justify-content: flex-end;">
            <div style="background: var(--accent-cyan); color: var(--bg-void); width: fit-content; padding: 4px 12px; border-radius: 8px; font-weight: 800; font-size: 0.65rem; margin-bottom: 8px;">CURRENT FOCUS</div>
            <h3 style="font-size: 1.6rem; font-family: 'Outfit'; font-weight: 800; text-shadow: 0 4px 10px rgba(0,0,0,0.5);">Goals of The Week</h3>
        </div>
    </div>
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
        ...
        <!-- COLUMN 1: OPERATIONS -->
        <div>
            <div class="dashboard-section-label">OPERATIONS <div class="dashboard-line"></div></div>
            <div id="weeks-mount" style="display: flex; flex-direction: column; gap: 15px;"></div>
        </div>

        <!-- COLUMN 2: COMMUNICATIONS -->
        <div>
            <div class="dashboard-section-label">RECAPS <div class="dashboard-line"></div></div>
            <div style="display: flex; flex-direction: column; gap: 12px;">
                <!-- Recap Card -->
                <div class="glass-medium active-glow recap-card"
                     id="recap-main-card"
                     style="min-height: 140px; border-radius: var(--radius-md); padding: 16px; position: relative; overflow: hidden; cursor: pointer;"
                     onclick="app.toggleWhisperSheet(this)">
                    ...
                    <div style="position: relative; display: flex; justify-content: space-between; align-items: flex-start;">
                        <h4 style="font-size: 1.1rem; font-family: 'Outfit'; font-weight: 800; color: white;">Recap of<br>Call</h4>
                        ...
                        <div class="whisper-carousel">
                            ...
                            <div class="whisper-date whisper-ghost-3 whisper-color-3" data-index="3"
                                 oncontextmenu="event.preventDefault(); event.stopPropagation(); app.editActiveWhisperDate()">MM/DD</div>
                    ...
                    <!-- WHISPER TASK SHEET (Correctly Nested) -->
                    <div id="whisper-task-sheet" class="recap-task-rollout">
                        <div class="recap-task-row">
                            <div class="task-dot"></div>
                            <input type="text" placeholder="Priority Quest #1..." class="task-input" oninput="app.saveWhisperTask(0, this.value)">
                        </div>
                        <div class="recap-task-row">
                            <div class="task-dot"></div>
                            <input type="text" placeholder="Dream Objective #2..." class="task-input" oninput="app.saveWhisperTask(1, this.value)">
                        </div>
                        <div class="recap-task-row">
                            <div class="task-dot"></div>
                            <input type="text" placeholder="Vision Step #3..." class="task-input" oninput="app.saveWhisperTask(2, this.value)">
                        </div>
                        <div class="recap-task-row">
                            <div class="task-dot"></div>
                            <input type="text" placeholder="Core Task #4..." class="task-input" oninput="app.saveWhisperTask(3, this.value)">
                        </div>
                        <div class="recap-task-row">
                            <div class="task-dot"></div>
                            <input type="text" placeholder="Final Execution #5..." class="task-input" oninput="app.saveWhisperTask(4, this.value)">
                        </div>
                    </div>
                </div>
            </div>
