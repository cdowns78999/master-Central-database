// Warn before closing — "are you sure?" dialog
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Cache fetched metadata by playlist ID to avoid duplicate requests
    const metaCache = {};
    ...
    async function fetchSpotifyMeta(url) {
        const parsed = extractImportId(url);
        if (!parsed) return null;
        const { type, id } = parsed;

        // Return cached result if we already fetched this ID
        if (metaCache[id]) {
            console.log(`[Import] Cache hit: "${metaCache[id].title} // ${metaCache[id].owner}"`);
            return metaCache[id];
        }
        ...
        for (const makeProxy of PROXIES) {
            try {
                ...
                // Skip "Page not found" garbage
                if (!title || title === 'Page not found') continue;

                console.log(`[Import] Fetched: "${title} // ${owner || 'Spotify'}"`);
                metaCache[id] = { title, owner };
                return { title, owner };
            } catch (e) {
                console.log(`[Import] Proxy failed, trying next...`);
                continue;
            }
        }

        console.log(`[Import] All proxies failed for ${id}`);
        return null;
    }
    ...
    async function processImport() {
        ...
        for (const line of lines) {
            ...
            // Tab-separated: URL [tab] curator [tab] tag — columns 2+3 can be anything
            if (trimmed.includes('\t')) {
                ...
                const parsed = extractImportId(trimmed);
                if (!parsed) continue;
    ...
    if (importInput) {
        ...
        async function runImport() {
            if (importing) return;
            importing = true;
            try { await processImport(); }
            finally { importing = false; }
        }

        // Let browser handle paste naturally, then process after value is set
        importInput.addEventListener('paste', () => setTimeout(runImport, 150));
        importInput.addEventListener('keydown', (e) => {
            if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); runImport(); }
        });
    }

    function extractImportId(url) {
        if (!url) return null;
        const match = url.match(/(playlist|track|album|artist|episode|show)\/([a-zA-Z0-9_]+)/);
    ...
    function createPlaylistRow(playlist, index, total) {
        ...
        // Triple-click art box to delete row
        artBox.addEventListener('click', (e) => {
            if (e.detail === 3) {
                // Show subtle "?" then remove
                artBox.style.backgroundImage = 'none';
                artBox.textContent = '?';
                artBox.style.color = 'rgba(255,255,255,0.25)';
                artBox.style.fontSize = '18px';
                artBox.style.fontWeight = '700';
                row.style.opacity = '0';
                row.style.transform = 'translateX(-20px)';
                setTimeout(() => {
                    const idx = playlistData["Top 20 Overall"].indexOf(playlist);
                    if (idx > -1) playlistData["Top 20 Overall"].splice(idx, 1);
                    row.remove();
                    renderPlaylists();
                }, 400);
            }
        ...
        jsonInput.addEventListener('change', (e) => {
            ...
            reader.onload = (event) => {
                try {
                    ...
                    // New format: { playlists: [...], curated: [...] }
                    if (imported.playlists && Array.isArray(imported.playlists)) {
                        // Restore full database
                        playlistData["Top 20 Overall"] = imported.playlists;
                        renderPlaylists();

                        // Restore selection mode curated items
                        dropZone.innerHTML = '';
                        if (imported.curated && imported.curated.length > 0) {
                            imported.curated.forEach(name => {
                                const match = playlistData["Top 20 Overall"].find(p => p.name === name);
                                if (match) addSelectedPlaylist(match);
                            });
                            // Auto-open selection mode if there were curated items
                            if (currentMode !== 'selection') {
                                currentMode = 'selection';
                                selectionOverlay.classList.remove('hidden');
                                modeTrigger.style.borderColor = 'var(--spotify-green)';
                        ...
                        // Legacy format: just an array of playlists → load into drop zone
                        dropZone.innerHTML = '';
                ...
                } catch (err) { console.error('Load error:', err); alert('Error loading JSON file.'); }
            };
    ...
    // Sort by Curator A-Z / Z-A — main grid + drop zone
    let curatorAZ = true;
    if (actionSortCurator) actionSortCurator.addEventListener('click', () => {
        const dir = curatorAZ ? 1 : -1;
        const sorted = [...playlistData["Top 20 Overall"]].sort((a, b) =>
            dir * (a.curator || '').localeCompare(b.curator || '')
        );
    ...
    // Sort by Tag A-Z / Z-A — main grid + drop zone
    let tagAZ = true;
    if (actionSortTag) actionSortTag.addEventListener('click', () => {
        const dir = tagAZ ? 1 : -1;
        const sorted = [...playlistData["Top 20 Overall"]].sort((a, b) =>
            dir * (a.tag || '').localeCompare(b.tag || '')
        );
        renderPlaylists(sorted);
    ...
    // Sort by Name A-Z / Z-A — main grid + drop zone
    let nameAZ = true;
    if (actionSortName) actionSortName.addEventListener('click', () => {
        const dir = nameAZ ? 1 : -1;
        const sorted = [...playlistData["Top 20 Overall"]].sort((a, b) =>
    ...
    // =====================================================
    //  SAVE JSON — captures full database + selection state
    // =====================================================
    shareTrigger.addEventListener('click', () => {
        const curatedNames = Array.from(dropZone.querySelectorAll('.curated-item')).map(item => item.dataset.name);

        const saveData = {
            playlists: playlistData["Top 20 Overall"],
        ...
        a.setAttribute("download", "spotify_manager_save.json");
