document.addEventListener('DOMContentLoaded', () => {
    ...
    function createPlaylistRow(playlist, index) {
        ...
        row.innerHTML = `
            ...
            <!-- Column 2: Curator Name (Interactive) -->
        ...
        // Curator Edit Logic (Surgical & Subtle)
        const curatorVal = row.querySelector('.curator-value');
        curatorVal.addEventListener('focus', () => {
            if (curatorVal.textContent === 'curator name') {
                curatorVal.textContent = '';
            }
        });
        ...
        curatorVal.addEventListener('blur', () => {
            ...
            if (val === '') {
                curatorVal.textContent = 'curator name';
                playlist.curator = '';
            } else {
                playlist.curator = val;
            }
            // Sync with curated zone if it exists
    ...
    function addSelectedPlaylist(playlist, shouldSave = true) {
        ...
        // Tag ticker logic for curated area
        const curatedTag = tagContainer.querySelector('.subtle-tag');
        curatedTag.addEventListener('focus', () => {
            if (curatedTag.textContent === 'add tag') curatedTag.textContent = '';
        });

        curatedTag.addEventListener('input', () => {
            if (curatedTag.textContent.trim().length > 0) {
                showTagSuggestions(curatedTag, playlist.name);
            } else {
                hideTagSuggestions();
            }
        });
        ...
        curatedTag.addEventListener('blur', (e) => {
            ...
            if (val === '') {
                curatedTag.textContent = 'add tag';
                playlist.tag = '';
            } else {
                playlist.tag = val;
            }
        ...
        // Subtle Curator Edit (Selection Area)
        const curatedCurator = row.querySelector('.curator-value');
        curatedCurator.addEventListener('focus', () => {
            if (curatedCurator.textContent === 'curator name') curatedCurator.textContent = '';
        });

        curatedCurator.addEventListener('blur', () => {
            const val = curatedCurator.textContent.trim();
            if (val === '') {
                curatedCurator.textContent = 'curator name';
                playlist.curator = '';
            } else {
                playlist.curator = val;
            }

            // Sync back to master object (no render)
            const master = playlistData["Top 20 Overall"].find(p => p.name === playlist.name);
            if (master) {
                master.curator = playlist.curator;
                // Update master DOM row if it exists
                ...
                if (masterRow) {
                    masterRow.querySelector('.curator-value').textContent = playlist.curator || 'curator name';
                }
            }
        });
