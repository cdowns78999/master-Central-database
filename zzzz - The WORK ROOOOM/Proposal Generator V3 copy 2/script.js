...
function addService(service) {
    if (!state.orders.find(o => o.name === service.name)) {
...
function copyMasterProposal() {
    const text = `
PROPOSAL FOR: ${state.artist.name}
---
STRATEGY: ${dom.field1.value}
TIMELINE: ${dom.field2.value}
---
INVESTMENT: $${state.totals.grandTotal.toFixed(2)}
    `.trim();
    navigator.clipboard.writeText(text).then(() => alert("Master Strategy Copied!"));
}

function copyRawCode() {
    navigator.clipboard.writeText(dom.rawCodeContent.innerText).then(() => alert("Payload Copied!"));
}
