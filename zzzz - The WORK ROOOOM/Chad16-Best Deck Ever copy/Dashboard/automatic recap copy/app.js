const app = {
    ...
    renderWeeks() {
        ...
        mount.innerHTML = this.weeks.map((w, idx) => `
            <div class="glass-medium week-card" style="min-height: 140px; border-radius: 20px; padding: 16px; position: relative; overflow: hidden; cursor: pointer; animation: screenFade 0.5s ease-out forwards; animation-delay: ${idx * 0.1}s;">
                ...
                <div style="position: relative; height: 100%; display: flex; flex-direction: column; justify-content: space-between;">
                    <div>
                        <h4 style="font-size: 1.1rem; color: white;">${w.title}</h4>
                        <p style="font-size: 0.8rem; color: var(--accent-cyan); font-weight: 800; margin-top: 5px;">${w.date}</p>
                    </div>
                </div>
            </div>
        `).join('') + `
